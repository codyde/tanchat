...
const dummy = 1;
const index = reactStartServer.eventHandler(handleServerAction);
const serverFnManifest = _serverFnManifest;
async function handleServerAction(event) {
  const request = reactStartServer.toWebRequest(event);
    ...
    try {
      let result = await (async () => {
        ...
        if (method.toLowerCase() === "get") {
          ...
          payload2 = payload2 ? reactStartClient.startSerializer.parse(payload2) : payload2;
          return await action(payload2, signal);
        }
        const jsonPayloadAsString = await request.text();
        const payload = reactStartClient.startSerializer.parse(jsonPayloadAsString);
        if (isCreateServerFn) {
      ...
      return new Response(
        result !== void 0 ? reactStartClient.startSerializer.stringify(result) : void 0,
        {
          status: reactStartServer.getResponseStatus(reactStartServer.getEvent()),
          headers: {
      ...
      console.error(error);
      console.info();
      return new Response(reactStartClient.startSerializer.stringify(error), {
        status: 500,
        headers: {
          "Content-Type": "application/json"
        }
