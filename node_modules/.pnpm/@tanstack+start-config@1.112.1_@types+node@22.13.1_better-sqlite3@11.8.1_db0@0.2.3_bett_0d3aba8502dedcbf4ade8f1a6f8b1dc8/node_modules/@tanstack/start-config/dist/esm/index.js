...
export async function defineConfig(inlineConfig = {}) {
    ...
    const deploymentPreset = checkDeploymentPresetInput(configDeploymentPreset || 'node-server');
    ...
    const TanStackServerFnsPlugin = createTanStackServerFnPlugin({
        ...
        client: {
            getRuntimeCode: () => `import { createClientRpc } from '@tanstack/start/server-functions-client'`,
            replacer: (opts) => `createClientRpc('${opts.functionId}', '${serverBase}')`,
        },
        ssr: {
            getRuntimeCode: () => `import { createSsrRpc } from '@tanstack/start/server-functions-ssr'`,
            replacer: (opts) => `createSsrRpc('${opts.functionId}', '${serverBase}')`,
        },
        server: {
            getRuntimeCode: () => `import { createServerRpc } from '@tanstack/start/server-functions-server'`,
    ...
    let vinxiApp = createApp({
        ...
        routers: [
            ...
            },
            {
                ...
                // TODO: RSCS - enable this
                // worker: true,
                handler: importToProjectRelative('@tanstack/react-start-server-functions-handler'),
                plugins: () => {
